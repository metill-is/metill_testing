...
p <- coverage_data |>
  ...
  geom_segment(
    aes(
      x = mean,
      xend = mean,
      y = as.integer(flokkur_ordered) - 0.3,
      yend = as.integer(flokkur_ordered) + 0.3
    ),
    linewidth = 0.7,
    alpha = 0.4
  ) +
...
p_tab <- p + wrap_table(table, space = "fixed") +
  plot_annotation(
    ...
    subtitle = "Spáin var reiknuð 8. nóvember",
    caption = caption,
    theme = theme(
...
ggsave(
  plot = p_tab,
  filename = here("Figures", "election_prediction_sharing.png"),
  width = 8,
  height = 0.55 * 8,
  scale = 1.4,
  bg = "#fdfcfc"
