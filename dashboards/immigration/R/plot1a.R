make_plot1a <- function() {
  plot_dat <- d |>
    ...
    select(dags, land, value = p, samtals) |>
    mutate(
      colour = case_when(
  ...
  p1 <- plot_dat |>
    ...
    geom_text_interactive(
      aes(x = 0.86, label = str_c(land, " "), data_id = land),
      hjust = 1,
      size = 3.5
    ) +
    ...
    labs(
      x = NULL,
      y = NULL,
      subtitle = glue("Heildarbreyting í {month(max(plot_dat$dags), label = T, abbr = F)} {year(max(plot_dat$dags))}")
    )
  ...
  p2 <- plot_dat |>
    ...
    scale_y_continuous(
      ...
      labels = \(x) hlutf(x - 1),
      limits = c(NA, NA), guide = guide_axis_truncated(
        trunc_upper = 1.3
      ),
      trans = "log10"
    ) +
  ...
  p3 <- plot_dat |>
    filter(land == "Ísland") |>
    ggplot(aes(dags, samtals)) +
    geom_area(
      aes(group = land, fill = colour),
      alpha = 0.3
    ) +
    geom_line(
      aes(group = land),
    ...
    labs(
      x = NULL,
      y = NULL,
      subtitle = "Fólksfjöldi á Íslandi"
    )
