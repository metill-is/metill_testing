make_plot2b <- function() {
  start_date <- min(atvinna$dags)
  end_date <- max(atvinna$dags)
  point_size <- 2.2
  ...
  p3 <- plot_dat |>
    ...
    labs(
      x = NULL,
      y = NULL,
      subtitle = "Þróun í hlutfalli vinnandi meðal erlendra einstaklinga | Leiðrétt fyrir árstíðarsveiflum"
    )
